...
const Wearables = () => {
  ...
  const wearableProducts = useMemo(() => {
    return products.filter(p => {
      const category = (p.category || "").toLowerCase();
      return category.includes("wearable") || category.includes("wearables");
    });
  }, [products]);

  /* ---------------- FILTER LOGIC ---------------- */
  const filteredProducts = useMemo(() => {
    let result = [...wearableProducts];
    ...
    return result;
  }, [wearableProducts, filters, searchQuery, sortBy]);
        ...
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-4">
          <aside className="hidden lg:block">
            <div className="sticky top-28 max-h-[calc(100vh-7rem)] overflow-y-auto pr-1">
              <Filter products={wearableProducts} onChange={setFilters} />
            </div>
          </aside>
          ...
          <section className="lg:col-span-3">
            <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div className="w-full max-w-xs text-left">
                ...
                <h2 className="mt-1 text-2xl font-semibold text-slate-900">Wearables</h2>
